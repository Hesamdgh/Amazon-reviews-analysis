...
# --- Filter Score = 1 and sample 20,000 reviews ---
...
# --- Save outputs (preserve Topic_Label, drop Representative_Docs) ---
# At this point `topic_info` should already contain Topic_Label (created earlier).
# Do NOT reassign topic_info = topic_model.get_topic_info() here — that would erase Topic_Label.

# If Topic_Label is missing but all_labels exists, restore it:
if "Topic_Label" not in topic_info.columns:
    # If we created all_labels earlier, use it (trim/pad to match)
    if 'all_labels' in globals():
        n = len(topic_info)
        topic_info["Topic_Label"] = pd.Series(all_labels[:n] + [""] * max(0, n - len(all_labels)))
    else:
        topic_info["Topic_Label"] = ""

# Drop Representative_Docs if present
if "Representative_Docs" in topic_info.columns:
    topic_info = topic_info.drop(columns=["Representative_Docs"])

# Optional: move Topic_Label to the end for readability
cols = [c for c in topic_info.columns if c != "Topic_Label"] + ["Topic_Label"]
topic_info = topic_info[cols]

# Save cleaned CSV
csv_path = f"{OUTPUT_DIR}/topics_sample_reviews_clean.csv"
topic_info.to_csv(csv_path, index=False)
print(f"Saved clean topics table → {csv_path}")
